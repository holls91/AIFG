...
public class CollisionAvoidance implements IDynamicMovement {
	...
	//	Holds the collision radius of a character (we assume all character have the same radius here)
	...
	public Optional<SteeringOutput> getSteering() {
		...
		for(Kinematic target: targets){
			...
			relativeVel = target.getVelocity().subtract(character.getPosition());
